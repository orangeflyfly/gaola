const CaptureSystem = {
    ...
    spin() {
        if(isRouletteRunning) return; isRouletteRunning = true;
        document.getElementById('stop-btn').classList.add('hidden');
        let extra = Math.floor(Math.random() * 360);
        currentRotation += (1800 + extra);
        document.getElementById('wheel').style.transform = `rotate(${currentRotation}deg)`;
        setTimeout(() => {
            let angle = currentRotation % 360; let ball;
            ...
            ballIcon.innerText = ball.emoji; document.getElementById('result-msg').innerText = `丟出了 ${ball.name}！`;
            ballIcon.classList.add('catch-shake'); SoundSystem.play('capture_shake');
            setTimeout(() => {
                ballIcon.classList.remove('catch-shake');
                if((Math.random() * 100) <= ball.rate) {
                    document.getElementById('result-msg').innerText = "捕獲成功！"; SoundSystem.play('capture_success');
                    ...
                    document.getElementById('pay-ask-text').innerText = `是否支付 ${cost} 金幣印製卡片？`;
                    document.getElementById('confirm-pay-btn').onclick = () => App.buyPokemon(currentEnemy, cost);
                } else { document.getElementById('result-msg').innerText = "掙脫逃跑了..."; document.getElementById('fail-exit-btn').classList.remove('hidden'); }
            }, 1600);
        }, 3500);
    }
};
