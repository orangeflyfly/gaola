const BattleSystem = {
    ...
    async start(target) {
        ...
        currentEnemy.rarity >= 6 ? enemyCard.classList.add('rarity-6') : enemyCard.classList.remove('rarity-6');
        ...
        this.loop = setInterval(() => {
            ...
            playerATB += 0.45;
            enemyATB += (currentEnemy.rarity * 0.35);
    ...
    triggerAttack(attacker) {
        ...
        if (enemyHP <= 0 || playerHP <= 0) {
            ...
            setTimeout(() => { enemyHP <= 0 ? CaptureSystem.start() : App.reload(); }, 1200);
        } else {
            setTimeout(() => {
                attacker === 'player' ? playerATB = 0 : enemyATB = 0;
                isPaused = false;
            }, 1200);
        }
    }
